	...
	const sortRunningListByDistance = async () => {
		...
		const sortedList = [...filteredRunningList].sort((a, b) => {
			const distanceA = getDistance(
				...
				a.markers[0].latitude,
				a.markers[0].longitude
			);
			const distanceB = getDistance(
				currentLocation.latitude,
				currentLocation.longitude,
				b.markers[0].latitude,
				b.markers[0].longitude
			);
		...
		Toast.show({
			...
			position: 'top', // 상단에 표시
			topOffset: 40, // 상단에서 40px 만큼 떨어져서 표시
		});
	};
		...
		<View style={styles.container}>
			...
			>
				...
				<View style={styles.buttonContainer}>
					...
					<TouchableOpacity
						onPress={sortRunningListByDistance}
						style={styles.distanceSortButton}
					>
						<Image
